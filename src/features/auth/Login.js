...
const getUserByNameAPI = async ( username) => {
  const options = {
      'method': 'POST',
      'url': `https://whiteboard-server-7kf51co6i-devon0216.vercel.app//users/useridByName`,
      'headers': {
...
const [userInfo, setuserInfo] = useState({ username: "", password: "" });
  ...
  const submit = async (e) => {
    ...
    if (username.length > 0 && password.length > 0) {
      const response = await fetch("https://whiteboard-server-7kf51co6i-devon0216.vercel.app//", {
        method: "POST",
        credentials: "include",
        headers: {
          "Content-type": "application/json",
